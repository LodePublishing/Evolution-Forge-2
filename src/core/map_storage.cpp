...
template<> boost::shared_ptr<Map> NodeCreator<Map>::createObjectFromNode(const std::vector<Node>& node) {
...
template<> Node NodeCreator<Map>::createNodeFromObject(const Map& t) {
	Node n("map");
	n.addChild(Node("id", boost::lexical_cast<std::string>(t.getId())));
	n.addChild(Node("name", t.getName()));

	Node locationVector("locations");
	Node pathList("paths");
