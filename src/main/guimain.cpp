...
void Main::draw(DC* const dc) const
{
	if(mainWindow->isShown())
	{
		...
		{
			SDL_Rect rc;
			rc.x = 0;rc.y = 0; rc.w = Object::max_x; rc.h = Object::max_y;
			if(efConfiguration.isBackgroundBitmap())
				dc->Blit(Object::theme.lookUpBitmap(BACKGROUND_BITMAP), rc);
			else
				dc->clearScreen();
		}
		mainWindow->draw(dc);
...
//		(BoGraphWindow*)(game[1]->window[BO_GRAPH_WINDOW])->mouseHasMoved();
	...
	{
		if((Button::getCurrentButton()!=NULL)&&(Button::getCurrentButton()->getAbsoluteRect().Inside(p)))
		{
	...
	if((Button::getCurrentButton())&&(!Button::getCurrentButton()->getAbsoluteRect().Inside(p)))
	{
...
// ignore mousemove if button is still pressed		
	if(Button::getCurrentButton())
	{
		if(Button::isCurrentButtonPressed())
		{
			Button::getCurrentButton()->forceHighlighted();
//			Button::getCurrentButton()->mouseHasMoved();
	...
	{
		for(unsigned int i=MAX_GAME;i--;)
			if((game[i])&&(!Button::getCurrentButton()))
				Button::setCurrentButton( (Button*) (game[i]->checkHighlight()) );
				
		if(!Button::getCurrentButton())
			Button::setCurrentButton ( (Button*) (mainWindow->checkHighlight()) );
		if(!Button::getCurrentButton())
			Button::setCurrentButton ( (Button*) (settingsWindow->checkHighlight()) );
		if(!Button::getCurrentButton())
			Button::setCurrentButton ( (Button*) (helpWindow->checkHighlight()) );
//		if(!Button::getCurrentButton())
//			Button::setCurrentButton ( (Button*) (mapWindow->checkHighlight()) );
	} else
	if((!Button::getCurrentButton())&&(Object::editTextField!=NULL))
		Button::setCurrentButton ( (Button*) (Object::editTextField->checkHighlight()) );
	if(Button::getCurrentButton())
	{
		Button::getCurrentButton()->mouseHasEnteredArea();
		Button::setCurrentButtonHasAlreadyLeft(false);
...
// TODO
		{
			...
			if(/*(temp!=NULL)&&*/(Object::toolTipParent==NULL))
				Object::tooltip=NULL;
			else
				Object::tooltip=new ToolTip(mainWindow, (Object::toolTipParent)->getToolTipString());
			mainWindow->setNeedRedrawNotMoved();
		}
	} else if(Object::tooltip)
	{
		delete Object::tooltip;
		Object::tooltip=NULL;
		mainWindow->setNeedRedrawNotMoved();
