...
BoGraphWindow::BoGraphWindow(Object* bograph_parent, const unsigned int game_number, const unsigned int game_max, const unsigned int player_number, const unsigned int player_max) :
	Window(bograph_parent, BOGRAPH_WINDOW_TITLE_STRING, theme.lookUpPlayerRect(BUILD_ORDER_GRAPH_WINDOW, game_number, game_max, player_number, player_max), theme.lookUpPlayerMaxHeight(BUILD_ORDER_GRAPH_WINDOW, game_number, game_max, player_number, player_max), SCROLLED, AUTO_SIZE_ADJUST, Rect(0, 15, 1000, 1000)),
	markAni(0),
	anarace(NULL),
	selectedItems(),
...
{
	setOriginalRect(Object::theme.lookUpPlayerRect(BUILD_ORDER_GRAPH_WINDOW, gameNumber, gameMax, playerNumber, playerMax));
	setMaxHeight(Object::theme.lookUpPlayerMaxHeight(BUILD_ORDER_GRAPH_WINDOW, gameNumber, gameMax, playerNumber, playerMax));
...
// go through the rectangles	
	...
	for(std::list<PROGRAM>::const_iterator order = anarace->getProgramList().begin(); order != anarace->getProgramList().end(); ++order, ++number)
		if(anarace->getTimer() + order->getBT() <= order->getTime())
		{
			...
			{
				os << "$" << formatTime(order->getRealTime(), efConfiguration.getGameSpeed()) << " - " << formatTime(order->getRealTime() + order->getBT(), efConfiguration.getGameSpeed()) << "$: " << GAME::lookUpUnitString(my_race, my_unit) << "#" << "  " << Object::theme.lookUpFormattedString(BOWINDOW_BUILD_TIME_STRING, GAME::race[my_race].stats[my_unit].BT/(efConfiguration.getGameSpeed()*3+6)) << "#";
...
void BoGraphWindow::draw() const
{
	...
	dc->setFont(Object::theme.lookUpFont(SMALL_FONT));
