...
BoDiagramWindow::BoDiagramWindow(Object* bod_parent, const unsigned int game_number, const unsigned int game_max, const unsigned int player_number, const unsigned int player_max) :
	Window(bod_parent, BODIAGRAM_WINDOW_TITLE_STRING, theme.lookUpPlayerRect(BUILD_ORDER_DIAGRAM_WINDOW, game_number, game_max, player_number, player_max), theme.lookUpPlayerMaxHeight(BUILD_ORDER_DIAGRAM_WINDOW, game_number, game_max, player_number, player_max), NOT_SCROLLED),
	diagram(new Diagram(this, Rect(getRelativeClientRectPosition() + Point(0,10), getClientRectSize() - Size(0,10)), Size(), DO_NOT_ADJUST)),
	anarace(NULL),
	bold(false),
	mouseTime(0),
	totalTime(0),
...
{
	setOriginalRect(Object::theme.lookUpPlayerRect(BUILD_ORDER_DIAGRAM_WINDOW, gameNumber, gameMax, playerNumber, playerMax));
	setMaxHeight(Object::theme.lookUpPlayerMaxHeight(BUILD_ORDER_DIAGRAM_WINDOW, gameNumber, gameMax, playerNumber, playerMax));
...
//	if(anarace->getProgramList().size()==0)
	...
	std::vector<std::list<GraphPoint> >& graph_list = diagram->getGraphList();
...
// ?? TODO
	...
	if(totalTime>0)
	{
		std::list<GraphPoint>::iterator k = graph_list[i].begin();
		++k;
		unsigned int id = 0;
	...
	for(std::list<GraphPoint>::iterator item = graph_list[i].begin(); item != graph_list[i].end(); ++item)
	{
