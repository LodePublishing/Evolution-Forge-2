...
BoMenu::BoMenu(Object* bo_parent, const Rect bo_rect, const Size distance_bottom_right, const ePositionMode position_mode):
	Menu(bo_parent, bo_rect, distance_bottom_right, position_mode, true, ONE_COLOUMN_MENU),
	number(0),
	anarace(NULL),
	lastRace(0),
	lastGoal(NULL)
...
{
	resetData();
	Object::reloadOriginalSize(); // => custom widths
}
...
// create new entries
	for(;i<number;++i)
	{
		...
		if(Object::focus == this)
			os << " [" << i << "]";
		MenuEntry* entry = new MenuEntry(this, Rect(Point(10, i * (FONT_SIZE + 9)), Size(getParent()->getWidth()-40, FONT_SIZE+7)), os.str());
		if(i==0)
