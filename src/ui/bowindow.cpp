...
BoWindow::BoWindow(Object* bo_parent, const unsigned int game_number, const unsigned int game_max, const unsigned int player_number, const unsigned int player_max) :
	Window(bo_parent, BOWINDOW_TITLE_STRING, theme.lookUpPlayerRect(BUILD_ORDER_WINDOW, game_number, game_max, player_number, player_max), theme.lookUpPlayerMaxHeight(BUILD_ORDER_WINDOW, game_number, game_max, player_number, player_max), SCROLLED, AUTO_SIZE_ADJUST, Rect(0, 30, 1000, 1000)),
	geneAnimation(0),
	anarace(NULL),
//	moveTarget(),
	optimizeMode(0),
...
//	boEndPoint(-1),
	...
	startLine(0),
	selectedItems(),
	menuGroup(new Group(this, Rect(Point(5, 5), Size(100,0)), Size(0, 5), HORIZONTAL_GROUP, false, TOP_LEFT, NULL_STRING)),
	restartBuildOrderButton(new Button(menuGroup, Rect(Point(0,0), Size(0,0)), Size(5,5), MENU_BUTTON, REFRESH_BITMAP, PRESS_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST)),
	saveBuildOrderButton(new Button(menuGroup, Rect(Point(0,0), Size(0,0)), Size(5,5), MENU_BUTTON, SAVE_BITMAP, STATIC_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST)),
	loadBuildOrderButton(new Button(menuGroup, Rect(Point(0,0), Size(0,0)), Size(5,5), MENU_BUTTON, LOAD_BITMAP, STATIC_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST)),

	// TODO in OptionenMenue
//	alwaysBuildWorker(new Button(this, Rect(5, 40, getWidth()-20, 10), Size(5, 5), CHECK_BUTTON, true, CHECK_BUTTON_MODE, SETTING_ALWAYS_BUILD_WORKERS_STRING, SETTING_ALWAYS_BUILD_WORKERS_TOOLTIP_STRING, SPECIAL_BUTTON_LEFT)),
//	onlySwapOrders(new CheckButton(this, Rect(5, 40, getWidth()-20, 15), Size(5, 5), ARRANGE_LEFT, SETTING_ONLY_SWAP_ORDERS_STRING, SETTING_ONLY_SWAP_ORDERS_TOOLTIP_STRING, false)), // TODO
//	unitMenuButton(new Button(this, Rect(Point(0, 0), Size(50,0)), Size(0,0), MY_BUTTON, true, STATIC_BUTTON_MODE, ADD_GOAL_STRING, DO_NOT_ADJUST, SMALL_BOLD_FONT, AUTO_HEIGHT_CONST_WIDTH)),
...
{
	setOriginalRect(Object::theme.lookUpPlayerRect(BUILD_ORDER_WINDOW, gameNumber, gameMax, playerNumber, playerMax));
	setMaxHeight(Object::theme.lookUpPlayerMaxHeight(BUILD_ORDER_WINDOW, gameNumber, gameMax, playerNumber, playerMax));
...
/*	std::list<PROGRAM> temp_program_list = anarace->getProgramList();
	...
	if((Button::getCurrentButton()!=NULL)&&(Button::isMoveByMouse()))
	{
		Rect current_position_rect = Button::getCurrentButton()->getRelativeRect();
		...
		Button::getCurrentButton()->resetGradient();
		...
		for(unsigned int row = startLine+1;
			entry!=boList.end(); 
			 ++entry, ++order, ++row)
		{
			if(Button::getCurrentButton() == (*entry))
			{
...
//	r2.setSize(Size(10, (unitMenu->getHeight()/2+1) * (FONT_SIZE+9)));
	...
	if(anarace->getTimer() + order->getBT() <= order->getTime())
	{
		...
		"  " << Object::theme.lookUpFormattedString(BOWINDOW_BUILD_TIME_STRING, GAME::race[my_race].stats[my_unit].BT/(efConfiguration.getGameSpeed()*3+6)) << "#";
			for(unsigned int i = RACE::MAX_RESOURCE_TYPES; i--;)
				if(GAME::race[my_race].stats[my_unit].resource[i] > 0)
				{
					...
					if(count > 1)
						os << " " << Object::theme.lookUpFormattedString(BOWINDOW_EACH_TOTAL_STRING, GAME::race[my_race].stats[my_unit].resource[i] * count/100);
			}
...
/*		if((Button::getCurrentButton()!=NULL)&&(Button::isMoveByMouse()))
		{
			Rect r = Button::getCurrentButton()->getRelativeRect();
			if((r.getBottom() >= edge.getTop()) && (r.getTop() <= edge.getBottom()))
			{
				row++;
				moveTarget = Rect(edge.getTopLeft() + ((Object*)getScrollBar())->getAbsolutePosition(), edge.getSize());
			}
		}*/
...
/*		if((unitMenuButton->isShown())&&(r2.getTop()<=edge.getBottom()))
		{
			if(addUnit>=0)
			{
				...
				BoEntry* t = new BoEntry((Object*)getScrollBar(), Point(getParent()->getWidth(), getRelativeClientRectPosition().y+205), Size(5,5),
				Object::theme.lookUpString((eString)(UNIT_TYPE_COUNT*my_race+my_unit)), *order); // (*anarace->getStartCondition())->getRace()?
...
/*		if((old_order->getRealTime()/60) > (last_time/60))
		{
			...
			if(timeSlot[last_time/60]==NULL)
				timeSlot[last_time/60] = new StaticText((Object*)getScrollBar(), os.str(), Rect(Point(7, 5+row*(FONT_SIZE+6)), Size(0,FONT_SIZE+5)), Size(5,5), FORCE_TEXT_COLOR, SMALL_BOLD_FONT, DO_NOT_ADJUST);
			else 
...
//		toErrorLog("check this bo");
		if(entry == boList.end())
		{
			...
			BoEntry* t = new BoEntry((Object*)getScrollBar(), Point(x_pos, 205), Size(5,5),
			// max size -y? TODO
		...
		if(((*entry)->program.getUnit() != my_unit)||((*entry)->getCount() != count))
		{
			...
			if(k != boList.end()) // => Found, move the entry
			{
				...
				++entry;
				boList.erase(k);
				if((edge != old->getTargetRect())&&((Button::getCurrentButton()!=old)||(!Button::isMoveByMouse())))
				{
			...
			} else // => not found, insert a new one
			{
				...
				if((gameMax>1)&&(gameNumber==0)) x_pos = - getWidth() - getParent()->getWidth()/2;
				BoEntry* t = new BoEntry((Object*)getScrollBar(), Point(x_pos, 205), Size(5,5), GAME::lookUpUnitString(my_race, my_unit), *order, count, id);
				new_item = true;
				t->setAllowMoveByMouse();
				t->setButtonColorsType(eButtonColorsType(UNIT_TYPE_0_BUTTON + GAME::race[my_race].stats[my_unit].unitType));
				if((edge != t->getTargetRect()) && ((Button::getCurrentButton()!=t) || (!Button::isMoveByMouse())))
				{
					t->adjustRelativeRect(edge);
					new_item = true;
				}
...
//		if((*entry)->getUnit() == my_unit)
			...
			{
				if((edge != (*entry)->getTargetRect())&&((Button::getCurrentButton()!=(*entry))||(!Button::isMoveByMouse())))
				{
...
/*	for(unsigned int i = (last_time/60)+1;i<MAX_TIME/60;i++)
	...
	if(new_item)
		Object::sound.playSound(SWISHIN_SOUND, (getAbsolutePosition() + getSize()/2).x);
	if(deleted_item)
		Object::sound.playSound(SWISHOUT_SOUND, (getAbsolutePosition() + getSize()/2).x);
}
...
/*		if((*entry)->isRightClicked())
		...
		if((Button::getCurrentButton() == (Button*)*entry)&&(Button::isMoveByMouse()))
		{
			(*entry)->setPosition((*entry)->getRelativeLeftBound(), ((*entry)->getRelativePosition() + mouse + Button::getMouseMovePoint() * (-1)).y);
			Button::setMouseMovePoint(mouse);
			reloadList = true;
...
/*	if((Button::getCurrentButton()!=NULL)&&(Button::isMoveByMouse())&&(moveTarget!=Rect(0,0,0,0)))
	{
...
void BoWindow::needSaveBox()
{
	...
	Window::saveBoxParameter.saveboxText = SAVE_BUILD_ORDER_AS_STRING;
	Window::saveBoxParameter.descriptionText = GIVE_BO_A_NAME_STRING;
...
//	database.saveBuildOrder(input_string, my_bo_list, anarace); ?
	boMenu->resetData();
	Object::msgList.push_back(Object::theme.lookUpFormattedString(SAVED_BUILD_ORDER_STRING, input_string));
