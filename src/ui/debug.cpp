...
DebugWindow::DebugWindow( Object* parent_window ) :
	Window( parent_window, DEBUG_WINDOW_TITLE_STRING, theme.lookUpGlobalRect(DEBUG_WINDOW), theme.lookUpGlobalMaxHeight(DEBUG_WINDOW), NOT_SCROLLED, NO_AUTO_SIZE_ADJUST, Rect(0, 10, 1280, 1024) ),
	framesPerSecond(10),
	generationsPerSecond(0),
	percentList(),
	tickGroup(new Group(this, Rect(Point(5, 10), Size(100,0)), Size(0,10), ONE_COLOUMN_GROUP, false, TOP_LEFT, NULL_STRING)),
	fpsText(new StaticText(this, Object::theme.lookUpString(FPS_TEXT_STRING), Rect(Point(0, 5), Size(0,0)), Size(10, 0), BRIGHT_TEXT_COLOR, SMALL_SHADOW_BOLD_FONT, TOP_RIGHT)),
	gpsText(new StaticText(this, Object::theme.lookUpString(GPS_TEXT_STRING), Rect(Point(50, 0), Size(0,0)), Size(10, 5), BRIGHT_TEXT_COLOR, SMALL_SHADOW_BOLD_FONT, DO_NOT_ADJUST /*BOTTOM_RIGHT*/))
{
	for(unsigned int i = 0; i < MAX_TICK_TYPES; i++)
		tickText[i] = new StaticText(tickGroup, Object::theme.lookUpString((eString)(DRAW_TICKS_STRING+i)), Rect(Point(0, 0), Size(0,0)), Size(0,0), BRIGHT_TEXT_COLOR, SMALL_SHADOW_BOLD_FONT, DO_NOT_ADJUST);
}
...
void DebugWindow::process()
{
	...
	for(std::list<unsigned int>::iterator i = percentList.begin(); i != percentList.end(); ++i, ++n)
	{
		...
		os << Object::theme.lookUpString((eString)(DRAW_TICKS_STRING+n)) << " " << *i << "%";
...
{
	setOriginalRect(Object::theme.lookUpGlobalRect(DEBUG_WINDOW));
	setMaxHeight(Object::theme.lookUpGlobalMaxHeight(DEBUG_WINDOW));
...
/*	unsigned int n = 0;
	...
	{
		dc->setPen(*Object::theme.lookUpPen((ePen)(tickBrush[n] - BRIGHT_UNIT_TYPE_0_BRUSH + UNIT_TYPE_0_PEN)));
		dc->setBrush(*Object::theme.lookUpBrush(tickBrush[n]));
