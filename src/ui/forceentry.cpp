...
ForceEntry::ForceEntry(Object* entry_parent, const Rect entry_rect, const std::string& entry_unit, const unsigned int unit_num, const eUnitType unit_type, GOAL* unit_goal):
	Button(entry_parent, entry_rect, Size(0,0), FORCE_ENTRY_BUTTON, NULL_BITMAP, PRESS_BUTTON_MODE, entry_unit, SPECIAL_BUTTON_LEFT, SMALL_SHADOW_BOLD_FONT, AUTO_HEIGHT_FULL_WIDTH),
	oldGoalCount(0),
	oldGoalTime(0),
	oldGoalFinalTime(0),
	startForce(0),
...
ForceEntry::~ForceEntry()
{
	if(ForceEntry::currentForceEntry == this)
	{
		...
		{
			if(Button::getCurrentButton() == makeTimeGoalButton)
				Button::resetButton();
		...
		{
			if(Button::getCurrentButton() == alwaysBuildButton)
				Button::resetButton();
			delete alwaysBuildButton;
		}
		if(buildMuchButton)
		{
			if(Button::getCurrentButton() == buildMuchButton)
				Button::resetButton();
		...
		if(!ForceEntry::timeEntryBox)
		{
			if(ForceEntry::currentForceEntry == NULL)
			{
				...
				if((rect_right.isTopLeftCornerInside(mouse)) && (ForceEntry::makeTimeGoalButton==NULL))
				{
					...
					ForceEntry::makeTimeGoalButton = new Button(getParent(), Rect(getRelativePosition() + Size(getParent()->getWidth()-52, 0), Size(16,10)), Size(0,0), GOAL_TIME_BUTTON, TIME_BUTTON_BITMAP, STATIC_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST);
					ForceEntry::alwaysBuildButton = new Button(getParent(), Rect(getRelativePosition() + Size(getParent()->getWidth()-34, 0), Size(10,10)), Size(0,0), CHECK_BUTTON, RADIO_OFF, CHECK_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST);
					ForceEntry::buildMuchButton = new Button(getParent(), Rect(getRelativePosition() + Size(getParent()->getWidth()-22, 0), Size(10,10)), Size(0,0), CHECK_BUTTON, RADIO_OFF, CHECK_BUTTON_MODE, NULL_STRING, DO_NOT_ADJUST);
					...
					Button::resetButton();
					Button::setCurrentButton(ForceEntry::makeTimeGoalButton);
				}
			} else
			// left the right side
			if(ForceEntry::currentForceEntry == this)
...
// situation 2: mouse is outside and timeEntryBox is not shown => Remove makeTimeGoalButton
			// EVTL PROBLEM: Ich loesche es hier, es ist aber Kind vom Parent das gerade noch proces durchlaeuft... evtl an parent benachrichtigen, dass button geloescht werden soll... :o
						if((Button::getCurrentButton() == ForceEntry::makeTimeGoalButton) || (Button::getCurrentButton() == ForceEntry::alwaysBuildButton) || (Button::getCurrentButton() == ForceEntry::buildMuchButton))
							Button::resetButton();
...
/*		Rect r = getAbsoluteRect();
		...
		{
			if(Button::getCurrentButton() == ForceEntry::makeTimeGoalButton)
				Button::resetButton();
			delete ForceEntry::makeTimeGoalButton;
			ForceEntry::makeTimeGoalButton = NULL;
		...
		if((ForceEntry::makeTimeGoalButton)&&(ForceEntry::makeTimeGoalButton->isLeftClicked()))
		{
			...
			{
				ForceEntry::timeEntryBox = new NumberField(getParent(), Rect(Point(getRelativePosition() + Size(getParent()->getWidth() - Object::theme.lookUpButtonWidth(LARGE_BUTTON_WIDTH)-40, 0)), Size(0, 0)), Size(0,0), DO_NOT_ADJUST, 0, coreConfiguration.getMaxTime(), NULL_STRING, NULL_STRING, 150, getTime(), false, TIME_NUMBER_TYPE);
				setOriginalSize(Size(getParent()->getWidth() - Object::theme.lookUpButtonWidth(SMALL_BUTTON_WIDTH)-40, getHeight()));
			}
...
// ------ Numbers
	...
	//dc->setFont(Object::theme.lookUpFont(SMALL_BOLD_FONT));
...
Button* ForceEntry::alwaysBuildButton = NULL;
Button* ForceEntry::buildMuchButton = NULL;
