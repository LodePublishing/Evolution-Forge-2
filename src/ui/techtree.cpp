...
TechTreeWindow::TechTreeWindow(Object* techtree_parent) :
	Window(techtree_parent, TECHTREE_WINDOW_TITLE_STRING, theme.lookUpGlobalRect(TECHTREE_WINDOW), theme.lookUpGlobalMaxHeight(TECHTREE_WINDOW), NOT_SCROLLED),
	unitChanged(false),
	currentGoalUnit(0),
	anarace(NULL),
	s(),
...
void TechTreeWindow::assignAnarace(ANABUILDORDER* techtree_anarace)
{
	...
	if(anarace == NULL)
	{
		...
		for(unsigned int i = RACE::UNIT_TYPE_COUNT;i--;)
			node[i] = new Object(this, Rect(Point((getWidth() - s.getWidth())/2, 15),s));
	}
...
{
	setOriginalRect(Object::theme.lookUpGlobalRect(TECHTREE_WINDOW));
	setMaxHeight(Object::theme.lookUpGlobalMaxHeight(TECHTREE_WINDOW));

	s = Size(Object::theme.lookUpButtonWidth(SMALL_BUTTON_WIDTH), FONT_SIZE);
	s2 = Size(Object::theme.lookUpButtonWidth(SMALL_BUTTON_WIDTH)+10, FONT_SIZE+2);
...
/*	for(unsigned int k = 0; k<10; ++k)
		if(tree.width[k]>0)
		{
			...
			for(std::list<unsigned int>::const_iterator i = tree.unit[k].begin(); i!=tree.unit[k].end(); ++i)
			{
				...
				std::string text = Object::theme.lookUpString((eString)(UNIT_TYPE_COUNT*anarace->getRace()+(*i)));
				Size text_size = dc->getTextExtent(text);
