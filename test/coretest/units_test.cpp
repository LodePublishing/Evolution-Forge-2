	...
	BOOST_AUTO_TEST_CASE (Units_constructor)
{
	...
	// unit to build
	boost::shared_ptr<Unit> test_unit4(new Unit(playerFixture.test_player, unitFixture.unitType2, mapFixture.test_map->getLocation(1), test_units, 1, test_construction_time, unit_list));
	...
	BOOST_CHECK_EQUAL(test_units->getLocalTotal(test_unit4->getLocalKey()), 1);
	...
	BOOST_CHECK_EQUAL(test_units->getGlobalTotal(test_unit4->getGlobalKey()), 1);
	// todo neutral
